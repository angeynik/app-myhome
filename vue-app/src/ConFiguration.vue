...
const ROOM_SENSOR_DEFS = [
  { key: 'dTemp',     label: 'Контроль температуры',                              max: 3 },
  { key: 'dHum',      label: 'Контроль влажности',                                max: 3 },
...
<style scoped>
/* Только то, чего нет в mainStyle.css */
...
/* Контент прокручивается внутри .body (flex:1, overflow-y:auto из mainStyle) */
.cfg-content {
  width: 100%;
  max-width: 680px;
  padding: 1rem;
...
.cfg-section {
  ...
  border: 1px solid rgba(224, 223, 231, 0.12);
  border-radius: 12px;
}
.cfg-room-section {
  min-height: auto;
...
.cfg-section--general {
  border-color: rgba(249, 123, 0, 0.35);
}
.cfg-section-header {
  display: flex;
  align-items: center;
...
.cfg-section-header--clickable {
  cursor: pointer;
  user-select: none;
  touch-action: manipulation;
}
.cfg-section-header--clickable:hover {
...
/* Шеврон */
.cfg-chevron-icon {
  stroke: var(--light_font);
  opacity: 0.55;
  transition: transform 0.22s;
  flex-shrink: 0;
}
.cfg-chevron-icon--open {
...
.cfg-subtitle {
  ...
  border-bottom: 1px solid rgba(249, 123, 0, 0.25);
}

/* Чекбокс + текст — переиспользует .checkbox-custom из mainStyle */
.cfg-checkbox-label {
  display: flex;
  align-items: center;
  gap: 0.6rem;
...
.cfg-submenu {
  ...
  border-left: 2px solid rgba(249, 123, 0, 0.3);
  margin-left: 0.4rem;
}
...
.cfg-btn-next:active { opacity: 0.8; }
...
.cfg-btn-finish:active { opacity: 0.82; }
